...
// Dummy data for Guest Packages
const guestPackages = [
  { _id: 'gp1', tier: 'Base', maxGuests: 100, price: 150 },
  { _id: 'gp2', tier: 'Premium', maxGuests: 200, price: 250 },
  { _id: 'gp3', tier: 'VIP', maxGuests: 500, price: 500 },
];

// Dummy data for Capture Limits
const captureLimits = [
  { _id: 'cl1', plan: 'Basic', planType: 'photos-only', photo: 15, video: 0, pricePerGuest: 0 },
  { _id: 'cl2', plan: 'Standard', planType: 'photos-videos', photo: 20, video: 1, pricePerGuest: 1 },
  { _id: 'cl3', plan: 'Premium', planType: 'photos-videos', photo: 25, video: 3, pricePerGuest: 2 },
  { _id: 'cl4', plan: 'Unlimited', planType: 'photos-only', photo: -1, video: 0, pricePerGuest: 5 },
];
...
export default function CreateEventPage() {
  ...
  const { user } = useUser();
  // Temporarily mock createEvent for UI development
  const createEvent = process.env.NODE_ENV === 'development'
    ? async (data: any) => {
        console.log("Mocking event creation:", data);
        return Promise.resolve("mock-event-id"); // Return a dummy ID
      }
    : useMutation(api.events.createEvent);
  ...
  return (
    <div className="min-h-screen grid grid-cols-[240px_1fr]">
      ...
      <aside className="bg-blue-600 text-white p-4 flex flex-col justify-between">
        {/* Top section: Logo and Nav Links */}
        <div>
          <h2 className="text-2xl font-bold mb-10">SNAPCLICKBANG</h2>
          <nav>
            <ul>
              <li className="mb-4">
                <Link href="/dashboard" className="flex items-center gap-3 text-lg font-medium hover:bg-blue-700 p-2 rounded-lg transition-colors">
                  <LayoutDashboard className="h-6 w-6" /> Dashboard
                </Link>
              </li>
              <li className="mb-4">
                <Link href="/events" className="flex items-center gap-3 text-lg font-medium bg-blue-700 p-2 rounded-lg transition-colors">
                  <CalendarIcon className="h-6 w-6" /> Events
                </Link>
              </li>
              <li className="mb-4">
                <Link href="/profile" className="flex items-center gap-3 text-lg font-medium hover:bg-blue-700 p-2 rounded-lg transition-colors">
        ...
        <div>
          <Button variant="ghost" className="w-full justify-start text-white text-lg font-medium hover:bg-blue-700 p-2 rounded-lg">
            <LogOut className="h-6 w-6 mr-3" /> Log out
          </Button>
        </div>
      </aside>
